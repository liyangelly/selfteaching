def caculator(): #定义计算器
    ...
    for i in str(formula01):
        if count > 1:
            print('输入有误，请重新输入')
            break
        elif i.isdigit() or i == '.': #isdigit检测字符串是否由数字组成
            num01 += i   #判断单个字符，对于连续的数字字符，将会组合在一起，即'789' 变为数字
            count = 0     #出现数字符号，计数器重置为0
        elif i == '*' or i == '/' or i == '+' or i == '-':
            num_list.append(float(num01)) #在列表末尾添加新的对象（float(num01))
            num01 = ''
            symbol.append(i)
        ...
        while "*" in symbol or "/" in symbol: #先寻找*和/运算
            for i in symbol:
                ...
                if i == "/":
                    j = symbol.index("/")
                    result = num_list[j] / num_list[j + 1]
                    del num_list[j:j + 2] #del删除的是变量而不是数据，仅当某对象的引用计数器为0时，该对象才会被从内存中释放
                    num_list.insert(j, result)
                    symbol.remove("/")
                    break
